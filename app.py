...
def viewall():
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, title, content, created_at FROM notes WHERE user_id=%s ORDER BY created_at DESC",
                (session['user_id'],))
    notes = cur.fetchall()
    cur.close()
    conn.close()
...
def viewnotes(note_id):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT id, title, content, created_at FROM notes WHERE id=%s AND user_id=%s",
...
def updatenote(note_id):
    ...
    cur.execute("SELECT id, title, content FROM notes WHERE id=%s AND user_id=%s",
                (note_id, session['user_id']))
    note = cur.fetchone()
